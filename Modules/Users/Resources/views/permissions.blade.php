...
@section('css')
    <link rel="stylesheet" href="{{ asset('/Modules/Users/Resources/assets/js/jstree/3.3.11/themes/default/style.min.css') }}" />
@endsection

@section('content')
    {{-- <div class="row no-gutters bg-100 rounded-soft px-card py-2 mt-2 mb-3">
        <div class="col d-flex align-items-center">
            <h5 class="mb-0">{{'Permisions'}}</h5>
        </div>
    </div> --}}
    <!-- <div class="row mb-2">
...
@section('js')
    <script src="{{ asset('/Modules/Users/Resources/assets/js/jstree/3.3.11/jstree.min.js') }}"></script>

    <script>
        $(function(){
            $.jstree.defaults.core.themes.icons = "";

            $.get($("meta[name='BASE_URL']").attr("content") + '/admin/permissions?type={{ $permissions["type"] }}&id={{ $permissions["id"] }}', function(response){
            ...
            // $('#permission-groups').on('click', '[data-name] .jstree-icon.jstree-checkbox', function(event, value = {loading: false}){
                ...
                $.post($("meta[name='BASE_URL']").attr("content") + "/admin/permissions/" + $('#permission-groups').attr('data-permission-id'), {
                    _method: "PUT",
