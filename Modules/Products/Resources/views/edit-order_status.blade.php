...
<form id="target" action="{{route('roles.store')}}" method="post" class="form-horizontal">
    ...
    <div class="form-group">
        <label  class="col-sm-2 control-label" for="" >{{ __('Status Type') }}</label>
        <div class="col-sm-10">
            <select name="type" id="type" class="form-control  ">
                <option value="1" @if ($type->type == '1') selected @endif >{{__('Driver')}}</option>
                <option value="2" @if ($type->type == '2') selected @endif >{{__('Vendor')}}</option>
            </select>
            <p class="invalid-feedback"></p>
        </div>
    </div>
...
<script>
    ...
    $.ajax({
        url: $("meta[name='BASE_URL']").attr("content") + '/admin/order_status/' + $id,
        type: 'PUT',
        data:data
    })
